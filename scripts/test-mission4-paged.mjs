...
assert.match(shell, /hideNext=\{currentPage===5\}/, "the ordinary Next button is hidden on the Lesson 4 quiz page");
assert.match(shell, /skipAction=\{currentPage===5\?\{label:t\("mission4\.skipQuiz"\),onClick:\(\)=>changePage\(6\)\}:null\}/, "Lesson 4 matches Lesson 1 with a localized Skip the quiz action");
...
assert.doesNotMatch(lab, /id="l4-watch-input"[^>]*disabled=/, "the prompt remains editable during and after generation");
assert.match(lab, /function updatePrompt\(value\)[\s\S]*setPrompt\(value\);/);
assert.doesNotMatch(lab.match(/function updatePrompt\(value\)[\s\S]*?\n  \}/)?.[0] ?? "", /clearGeneration|model\.cancel/, "editing only updates the draft while the current generation keeps running");
assert.doesNotMatch(lab, /starters\.map\(starter => <button[^>]*disabled=/, "starter examples remain clickable while generation is running");
assert.match(lab, /function chooseStarter\(starter\)[\s\S]*setPrompt\(starter\.text\);/);
assert.doesNotMatch(lab.match(/function chooseStarter\(starter\)[\s\S]*?\n  \}/)?.[0] ?? "", /clearGeneration|model\.cancel/, "choosing an example only replaces the draft while the current generation keeps running");
assert.match(lab, /function runDraft\(\)[\s\S]*nextPrompt !== appliedPromptRef\.current[\s\S]*clearGeneration\(nextPrompt\)[\s\S]*runAuto/, "Run applies a changed draft and only then replaces the current generation");
assert.match(lab, /const defaultPrompt = t\("mission4\.live\.lab\.defaultPrompt"\)/, "the default prompt is locale-driven");
assert.match(lab, /STARTER_IDS\.map[\s\S]*mission4\.live\.lab\.starters/, "starter prompts use stable IDs and locale-driven display text");
assert.match(lab, /promptSourceRef[\s\S]*promptLockedRef[\s\S]*localizedPrompt/, "locale switching updates untouched presets without overwriting learner-authored or generated text");
...
assert.match(lab, /input_token_ids:tokenIdsRef\.current/, "generation continues with stable Token IDs instead of re-tokenising decoded fragments");
assert.match(lab, /const after = prediction\.next_decoded_text/, "the UI derives generated text from official full-sequence decoding");
assert.doesNotMatch(lab, /before \+ prediction\.selected\.raw_token/, "the UI never concatenates independently decoded Token strings");
assert.match(lab, /candidateLabel[\s\S]*byteFragment/, "incomplete byte Tokens receive a truthful localized technical label");
...
assert.match(lab, /setMessage\(\{ key:"stepMessage", params:\{ step:step\.step, token:selectedLabel \} \}\)/, "dynamic status stores a Unicode-safe label and interpolation parameters");
...
assert.match(modelService, /decodeStableTokenIds\(tokenizer, \[\.\.\.inputIds, tokenId\]\)/, "candidate display is derived from context-aware official decoding");
assert.match(modelService, /next_input_token_ids:nextInputTokenIds/, "the backend returns the exact ID-level generation state");
...
assert.match(modelConfig, /onnx-community\/Qwen2\.5-0\.5B/);
assert.doesNotMatch(modelConfig, /Instruct/, "the live completion lab uses the Qwen base model without chat templating");
assert.match(modelConfig, /dtype: "q4"/);
assert.match(modelConfig, /bae5ceaee026f0d0592858b2bd27645a06f19c42/);
...
assert.match(lab, /if \(autoLoopRef\.current\) return/, "overlapping automatic runs are disabled");
assert.doesNotMatch(lab, /mode:"greedy"/, "the production LivePredictionLab no longer forces argmax selection");
assert.match(lab, /mode:"sampling"[\s\S]*seed:TEACHING_SEED[\s\S]*sample_index:stepsRef\.current\.length/, "Auto and step-by-step share reproducible temperature sampling");
assert.match(modelService, /chooseTokenId\(probabilities, mode, createSeededRandom\(seed, sampleIndex\)\)/, "the backend samples the full scaled vocabulary with a deterministic stream");
assert.match(css, /calc\(100% - 18px\) 4px no-repeat/, "the temperature track ends under the thumb centres without endpoint tails");
...
const localeLanguages = ["en", "zh", "fr", "de"];
const mission4Locales = await Promise.all(localeLanguages.map((language) => read(`../src/locales/${language}/mission4.json`).then(JSON.parse)));
const english = mission4Locales[0];
...
for (const [index, locale] of mission4Locales.entries()) {
  const language = localeLanguages[index];
  assert.ok(locale.live.lab.defaultPrompt, `${language} has a localized default prompt`);
  assert.deepEqual(Object.keys(locale.live.lab.starters).sort(), ["cat", "dragon", "weather"], `${language} has the same stable starter IDs`);
}
assert.equal(new Set(mission4Locales.map((locale) => locale.live.lab.defaultPrompt)).size, 4, "all four locales provide a distinct default starter");
