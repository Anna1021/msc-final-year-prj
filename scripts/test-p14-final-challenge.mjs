...
assert.match(runtime, /contextChamber:[\s\S]*insideWindow: Object\.freeze\(\["curious", "reader", "opened", "the", "old", "book"\]\)/, "English Room 2 preserves its current-context answer group");
assert.match(runtime, /contextChamber:[\s\S]*insideWindow: Object\.freeze\(\["他", "打开了", "一本", "封面陈旧的", "故事书", "慢慢阅读"\]\)/, "Chinese Room 2 uses a complete natural Chinese context-window example");
...
assert.match(runtime, /data\.selectedOption === teaching\.correctOptionId[\s\S]*roomComplete\(room\)/, "Room 2 completes only for its stable current-context answer ID");
...
assert.match(runtime, /connectionLab:[\s\S]*id: "reader", label: "reader"[\s\S]*id: "opened", label: "opened"/, "English Room 3 uses the Lesson 3 sentence");
assert.match(runtime, /connectionLab:[\s\S]*id: "reader", label: "读者"[\s\S]*id: "opened", label: "打开了"/, "Chinese Room 3 uses a natural locale-specific sentence");
assert.match(runtime, /Object\.freeze\(\{ id: "reader", strength: 8 \}\)/, "reader has the strongest shown connection");
assert.match(runtime, /question: "Which token is shown contributing the most information to/, "Room 3 asks learners to read the shown pattern");
...
assert.match(runtime, /id: "book", label: "book", value: 46[\s\S]*id: "door", label: "door", value: 27[\s\S]*id: "box", label: "box", value: 15[\s\S]*id: "window", label: "window", value: 8[\s\S]*id: "other", label: "other", value: 4/, "Room 4 contains the fixed illustrative prediction totaling 100%");
...
assert.match(runtime, /correctOptionId: "most-likely"/, "Room 4 uses a stable semantic correct-answer ID");
...
assert.match(runtime, /The dragon sleeps in the <mark>cave<\/mark>[\s\S]*The dragon hides in the <mark>cave<\/mark>[\s\S]*The dragon waits in the <mark>cave<\/mark>[\s\S]*The dragon flew over the <mark>castle<\/mark>/, "English Room 5 uses the four requested simplified training records");
assert.match(runtime, /小龙在<mark>山洞<\/mark>里睡觉[\s\S]*小龙躲进了<mark>山洞<\/mark>[\s\S]*小龙在<mark>山洞<\/mark>里等待[\s\S]*小龙飞过了<mark>城堡<\/mark>/, "Chinese Room 5 uses pedagogically equivalent training records");
assert.match(runtime, /prompt: "The dragon returned to the ___"[\s\S]*id: "cave", label: "cave", value: 48[\s\S]*id: "castle", label: "castle", value: 29[\s\S]*id: "forest", label: "forest", value: 23/, "Room 5 reveals the requested illustrative toy prediction");
...
assert.match(runtime, /correctOptionId: "patterns"/, "Room 5 uses a stable semantic correct-answer ID");
