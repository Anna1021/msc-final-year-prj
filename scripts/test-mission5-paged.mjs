...
assert.equal(PAGED_MISSIONS[6].pages.length,8);
...
for(const page of[1,2,3,4,5,6,8])assert.match(shell,new RegExp(`Lesson5NewPage${page} active=\\{currentPage===${page}\\}`));
assert.match(shell,/Mission1PagedKnowledgeQuiz active=\{currentPage===7\}/,"Page 7 renders the shared Lesson 1 quiz framework");
...
assert.match(shell,/visited\.has\(pageCount\)/,"direct Page 8 access cannot complete Lesson 5");
...
assert.match(shell,/const pageHero=<LessonPageHero[\s\S]*lessonIndex=\{5\}[\s\S]*lessonCount=\{5\}[\s\S]*lessonName=\{t\("mission5\.shell\.topic"\)\}[\s\S]*title=\{t\(page\[1\]\)\}[\s\S]*subtitle=\{pageSubtitle\}/,"all eight Lesson 5 pages, including the summary, use the shared localized Hero");
...
assert.match(app,/"\/mission\/5-bias-paged": \{ number: 5, total: 8/,"the learner-facing TopBar also reports eight pages");
...
assert.match(pages,/export function Lesson5NewPage6[\s\S]*l5-page-six-steps[\s\S]*l5-page-six-growth/,"Page 6 bridges repeated next-Token prediction to a growing response");
assert.match(css,/l5-page-six-growth-row\{display:grid;grid-template-columns:repeat\(4,minmax\(100px,\.65fr\) 20px\) minmax\(100px,\.65fr\) 22px 20px minmax\(230px,1\.5fr\)/,"Page 6 gives all five growth cards and the final response their intended grid columns");
assert.match(css,/l5-page-six-growth-row \.is-final\{min-width:230px/,"the final response cannot collapse into an arrow-width column");
assert.match(pages,/export function Lesson5NewPage8[\s\S]*LessonSummaryPage[\s\S]*pageNumber="8"/,"Page 8 is a real shared-scaffold lesson summary");
...
const page5Source=pages.match(/function ToyLocationVisual[\s\S]*?const PAGE_6_FIRST_CANDIDATES/)?.[0]||"";
...
for(const number of[1,2,3,4])assert.match(pages,new RegExp(`mission5\\.page8\\.idea${number}Title`),`Page 8 includes summary idea ${number}`);
assert.doesNotMatch(pages,/Lesson5PendingPage|l5-route-placeholder/,"Page 8 no longer renders a placeholder or duplicate legacy heading");
...
assert.match(shell,/import "\.\.\/mission1\/mission1Paged\.css"/,"Page 7 reuses the established Lesson 1 quiz CSS");
assert.match(shell,/hideNext=\{currentPage===7\}/,"the shell Next button is hidden while the quiz controls progression");
assert.match(shell,/skipAction=\{currentPage===7\?\{label:t\("mission5\.skipQuiz"\),onClick:\(\)=>changePage\(8\)\}:null\}/,"Lesson 5 matches Lesson 1 with a localized Skip the quiz action");
assert.match(shell,/onContinue=\{\(\)=>changePage\(8\)\}[\s\S]*pageNumber=\{7\}/,"quiz completion continues directly to Page 8 and displays badge 7");
assert.match(shell,/quizResult==="correct"/,"Lesson 5 completion includes successful quiz completion");
assert.match(shell,/currentPage===7\?"mission-1-paged mission-2-paged lesson-quiz-layout"/,"Page 7 opts into the same scoped quiz presentation as the existing lessons");
...
assert.equal(locales[0].navigation.page6Title,"From one token to a full response");
assert.match(locales[0].navigation.page6Subtitle,/repeating that process[\s\S]*longer response/);
assert.equal(locales[0].navigation.page7Title,"Check what you learned");
assert.equal(locales[0].navigation.page8Title,"Lesson summary");
assert.match(locales[0].page6.intro,/Autoregressive language models[\s\S]*predicting one next Token/);
assert.match(locales[0].page6.support.prediction.body,/one next Token at a time/);
