...
export default function Mission1PagedKnowledgeQuiz({active,copy,resetKey,onResultChange,onContinue,pageNumber=5,lessonClassName="mission-1-paged__lesson",showLesson1Visuals=true,showDecoration=true,showCorrectAnswer=false,showTitle=true,guideTarget,guideCheckTarget}){
  ...
  if(finished)return <section className={`course-section ${lessonClassName} mission-knowledge-quiz`} data-lesson-page={pageNumber} data-guide-target={guideTarget} hidden={!active}><QuizHeading copy={copy} pageNumber={pageNumber} showDecoration={showDecoration} showTitle={showTitle}/><div className="mission-checkpoint-complete" role="status"><span className="mission-checkpoint-complete__icon"><CheckCircle2/></span><h3>{copy.niceWork}</h3><p>{copy.completionText}</p><div>{copy.completedIdeas.map(item=><span key={item}><Check/>{item}</span>)}</div><button type="button" className="primary" onClick={onContinue}>{copy.continue}<ArrowRight/></button></div></section>;

  return <section className={`course-section ${lessonClassName} mission-knowledge-quiz`} data-lesson-page={pageNumber} data-guide-target={guideTarget} hidden={!active}>
