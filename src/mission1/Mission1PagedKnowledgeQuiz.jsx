...
export default function Mission1PagedKnowledgeQuiz({active,copy,resetKey,onResultChange,onContinue,pageNumber=5,lessonClassName="mission-1-paged__lesson",showLesson1Visuals=true}){
  ...
  useEffect(()=>{setCurrentQuestion(0);setLatestQuestion(0);setSelected(null);setAnswerState("idle");setCompleted([]);setResponses({});onResultChange("idle")},[resetKey,onResultChange]);
  ...
  if(finished)return <section className={`course-section ${lessonClassName} mission-knowledge-quiz`} data-lesson-page={pageNumber} hidden={!active}><QuizHeading copy={copy} pageNumber={pageNumber}/><div className="mission-checkpoint-complete" role="status"><span className="mission-checkpoint-complete__icon"><CheckCircle2/></span><h3>{copy.niceWork}</h3><p>{copy.completionText}</p><div>{copy.completedIdeas.map(item=><span key={item}><Check/>{item}</span>)}</div><button type="button" className="primary" onClick={onContinue}>{copy.continue}<ArrowRight/></button></div></section>;

  return <section className={`course-section ${lessonClassName} mission-knowledge-quiz`} data-lesson-page={pageNumber} hidden={!active}>
    <QuizHeading copy={copy} pageNumber={pageNumber}/>
...
function QuizHeading({copy,pageNumber}){return <><div className="mission-paged-page-heading"><div className="course-section-head"><h2><span>{pageNumber}</span>{copy.quizTitle}</h2></div><span className="mission-paged-heading-sticker mission-checkpoint-clipboard" aria-hidden="true"><CheckCircle2/></span></div><div className="mission-checkpoint-banner"><div><span><HelpCircle/>{copy.conceptCheckpoint}</span><h3>{copy.threeQuestions}</h3><p>{copy.answerEach}</p></div><div className="mission-checkpoint-decoration" aria-hidden="true"><strong>?</strong><img src="/assets/img/mission-robot-pointing.png" alt=""/></div></div></>}
