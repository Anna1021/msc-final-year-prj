...
export const LESSON_3_PAGES = Object.freeze([
  ["equal","mission3.page1.title","mission3.page1.subtitle"],
  ["task","mission3.page2.title","mission3.page2.subtitle"],
  ["combine","mission3.page3.title","mission3.page3.subtitle"],
  ["order","mission3.page4.title","mission3.page4.subtitle"],
  ["practice","mission3.page5.title","mission3.page5.subtitle"],
...
const REQUIRED = ["task-focus","combined-clues","clue-practice"];

export default function Lesson3Paged({ setProgress, navigate, notify }) {
  const { t } = useI18n();
  const initial = parseLessonPage(window.location.search, LESSON_3_PAGES.length);
  ...
  useEffect(()=>{
    const pop=()=>{const next=parseLessonPage(window.location.search,LESSON_3_PAGES.length);setCurrentPage(next);setVisited((current)=>new Set(current).add(next));};
    window.addEventListener("popstate",pop); return()=>window.removeEventListener("popstate",pop);
  },[]);

  function changePage(next) {
    const safe=Math.min(LESSON_3_PAGES.length,Math.max(1,next));
